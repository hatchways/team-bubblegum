...
import csv, calendar, base64
from sendgrid.helpers.mail import (Mail, TemplateId, Attachment, FileContent, FileName, FileType, Disposition, ContentId)
import datetime as dt

@celery.task
def download_csv(year, month, user_id):
    with app.app_context():
        user = User.query.get(user_id)
        user_receipts = Receipt.query.filter_by(user_id=user_id).filter(extract('year', Receipt.receipt_date) == year).filter(extract('month', Receipt.receipt_date) == month).all()
        try:
            headers = ['title', 'amount', 'category', 'receipt_date']
            with open('manual-report.csv', 'w') as csvfile:
        ...
        with open('manual-report.csv', 'rb') as csvfile:
            data = csvfile.read()
        ...
        try:
            ...
            return "Failed to send monthly overview"
        return "Email sent"
